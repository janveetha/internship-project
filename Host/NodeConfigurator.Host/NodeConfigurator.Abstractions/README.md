...
Shared contract interfaces and models for building NodeConfigurator extensions.

## Overview

This package contains the **stable API contract** that plugins implement and the host consumes. Both the host application and all plugin assemblies reference this same package to ensure type compatibility at runtime.

## Key Types

| Type | Purpose |
|------|---------|
| `IExtension` | Core plugin interface: identity, lifecycle, UI component, serialization, validation |
| `IExtensionContext` | Host services provided to plugins: event bus, shared state, nodes, save coordinator |
| `IDetailContributor` | Optional: contribute read-only details to a node's info panel |
| `INodeEditorContributor` | Optional: render an inline editor on a node's page |
| `IEventBus` / `BusMessage` | Loosely-coupled cross-extension messaging |
| `ISharedStateStore` | Cross-extension shared values (selected node, central node, mode) |
...
{
	public string Id => "ext.myextension";
	public string DisplayName => "My Extension";
	public Type ComponentType => typeof(MyExtensionView);
...
- **MAJOR** (1.x.x → 2.0.0): Breaking changes — plugins must be rebuilt
- **MINOR** (1.0.x → 1.1.0): New features, backward compatible — old plugins still work
- **PATCH** (1.0.0 → 1.0.1): Bug fixes only — no API changes

The `IExtension` contract is assumed identical across the host and all plugins. There is no runtime contract-version gating: a plugin always loads regardless of any declared version. Differing third-party dependency versions are handled by per-plugin `AssemblyLoadContext` isolation.
